...
public:
  ...
  bool LogLineEnabled() { return log_line; }
  bool LogFileEnabled() { return log_file; }
...
#define LOG(logger, message) {                                      \
	ostringstream os;                                               \
	if (logger.LogLineEnabled() && logger.LogFileEnabled()) {       \
		os << __FILE__ << ":" << __LINE__ << " " << (message) << "\n";  \
	}                                                               \
	else if (!logger.LogLineEnabled() && logger.LogFileEnabled()) { \
		os << __FILE__ << " " << (message) << "\n";                 \
	}                                                               \
	else if (logger.LogLineEnabled() && !logger.LogFileEnabled()) { \
