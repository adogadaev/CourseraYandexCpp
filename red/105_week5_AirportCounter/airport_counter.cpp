...
private:
	static const size_t AIRPORTS_NUM = static_cast<size_t>(TAirport::Last_);

public:
	using Item = std::pair<TAirport, size_t>;
	using Items = const array<Item, AIRPORTS_NUM>&;

private:
	array<Item, AIRPORTS_NUM> adata_;

public:
	// конструктор по умолчанию: список элементов пока пуст
	AirportCounter() {
		for (size_t i = 0; i < AIRPORTS_NUM; i++) {
			adata_[i] = {(TAirport)i, 0};
		}
	}
	...
	size_t Get(TAirport airport) const {
		return adata_[(size_t)airport].second;
	}

	// добавить данный элемент
	void Insert(TAirport airport) {
		adata_[(size_t)airport].second++;
	}

	// удалить одно вхождение данного элемента
	void EraseOne(TAirport airport) {
		size_t idx = (size_t)airport;
		if (adata_[idx].second > 0) {
			adata_[idx].second--;
		}
	}
	...
	void EraseAll(TAirport airport) {
		adata_[(size_t)airport] = {airport, 0};
	}
